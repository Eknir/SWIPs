...
```json
{
   "honeyPrices": {
       "TTL": <TTLValue>,
       "price": {
           "validFrom0": <honeyPriceWei0>,
           "validFromN": <honeyPriceWeiN>
       }
   }
}
```
...
* The node querying the price applies the price by looking up validFrom which is in the most recent past.
* Upon receipt of payment or intent to send a payment, nodes will check their local cache for the honeyPrice object. If it is not valid anymore or non-existant, the oracle will be queried. Otherwise, the cached price will be aplied.
...
An on-chain oracle *might* give a different answer to two nodes when queried at the same time if there exists a (temporary) fork in the blockchain. It it appears that such forks are common and cause inbalances to reach the disconnect treshhold for a non-insignificant amount of nodes any oracle price-update might only be considered valid if it happened a sufficient amount of time in the past (usually 12 blocks in Ethereum)
 
## Backwards Compatibility
<!--All SWIPs that introduce backwards incompatibilities must include a section describing these incompatibilities and their severity. The SWIP must explain how the author proposes to deal with these incompatibilities. SWIP submissions without a sufficient backwards compatibility treatise may be rejected outright.-->
Currently, the Swarm is not running with a live test net for setting prices, so no problems are expected with this SWIP if it is implemented before the Swarm will go live with price-incentivization.
